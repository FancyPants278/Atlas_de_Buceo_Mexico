...
- "shp/MPA_MX_v1.0_01032021/final_mpa_layer.shx" 
- "shp/MPA_MX_v1.0_01032021/final_mpa_layer.prj" 
- "shp/MPA_MX_v1.0_01032021/final_mpa_layer.dbf" 
- "shp/MPA_MX_v1.0_01032021/final_mpa_layer.cpg" 
...
ds <- shapefile("shp/Diving_sites_v1.0_01032021/dive_sites.shp", verbose = FALSE)
to <- shapefile("shp/Tourism_operators_v1.0_01032021/operators.shp", verbose = FALSE)

# Loading MPA layer -------------------

mpa_layer <- st_read("shp/MPA_MX_v1.0_01032021/final_mpa_layer.shp") %>% 
        select(ANP, ID)


mpa_metadata <- read.csv("data/MPA_metadata_v.1.0_01032021.csv", na.strings = "") # MPA metadata

final_mpa <- merge(mpa_layer, mpa_metadata, by = c("ID", "ANP")) %>%
        
        mutate(Pesca = replace_na(Pesca, "NA"),
               Buceo = replace_na(Buceo, "NA")) %>%
        mutate(
                Pesca = factor(
                        Pesca,
                        levels = c("NA", "ND", "NO", "SI"),
                        labels = c("Sin manejo", "No especificado", "Prohibida",
                                   "Permitida")
                ),
                Buceo = factor(
                        Buceo,
                        levels = c("NA", "ND", "NO", "SI"),
                        labels = c("Sin manejo", "No especificado", "Prohibido", "Permitido")
                )
        )


final_mpa <- st_zm(final_mpa, drop = TRUE, what = "ZM")


mpa <- as(final_mpa, "Spatial")
...
mpa$labels3 <- paste0("<strong> Name: </strong> ", mpa$ANP, "<br/> ",
                      ...
                      "<strong> Zona: </strong> ", mpa$Categoria, "<br/> ",
                      "<strong> Subzona </strong> ", mpa$Subzonific, "<br/> ",
